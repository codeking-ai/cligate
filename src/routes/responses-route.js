...
export async function handleResponses(req, res) {
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
        if (rotator.isAllRateLimited(modelId)) {
            ...
            if (minWait > MAX_WAIT_BEFORE_ERROR_MS) {
                return res.status(429).json({
                    error: { message: `All accounts rate-limited. Wait ${Math.round(minWait / 1000)}s` }
                });
            }
        ...
        if (!account) {
            ...
            return res.status(401).json({ error: { message: 'No available accounts in proxy pool.' } });
        }
        ...
        try {
            // Forward raw body as-is (preserving zstd/gzip encoding)
    ...
    return res.status(503).json({ error: { message: 'Max retries exceeded.' } });
}
