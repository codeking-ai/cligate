...
- Voice input now works **offline**: the in-browser Whisper model is bundled inside the desktop app (downloaded at build time from ModelScope's `Xenova/whisper-tiny` ONNX mirror — reachable where `huggingface.co` is blocked) and loaded locally from `/models/`, so the keyless fallback no longer fetches the model from Hugging Face at runtime. Build pulls it via `npm run fetch:stt-model` (run automatically in CI before packaging); the ~43MB model is not committed to git.

### Fixed

- Microphone errors are now reported by their real cause (permission denied vs. no device vs. device busy vs. insecure context) instead of always showing "denied", and transcription failures are logged to the console with the failing stage so they can be diagnosed.
...
- Added voice input to the chat composer: click the microphone to record, click again to transcribe into the message box. Prefers server-side transcription through your configured OpenAI/Gemini key pool (`gpt-4o-mini-transcribe`, falling back to `whisper-1`, or Gemini), and automatically falls back to fully in-browser Whisper (transformers.js) when no key is available. The Whisper model ships bundled inside the desktop app (downloaded at build time from ModelScope via `npm run fetch:stt-model`, which `electron:build` runs automatically) and is loaded locally, so the keyless path transcribes offline with no huggingface.co/CDN fetch — important where Hugging Face is blocked. See `docs/voice-recognition-design.zh-CN.md`.
